...
document.getElementById("btn-login")?.addEventListener("click", async () => {
  ...
  try {
    ...
    if (!res.ok) {
      const errorMessage = data?.response?.errorMessage || "Erro ao fazer login";
      setMsg("login-msg", errorMessage);

      // If email not verified, offer to go to verify screen
      if (isEmailNotVerifiedError(errorMessage)) {
        handleLoginEmailNotVerified(usuario);
        const loginMsgEl = document.getElementById("login-msg");
        if (loginMsgEl) {
          loginMsgEl.innerHTML = errorMessage +
            ' <a href="#verify" class="link">Verificar agora</a>';
        }
      }
      return;
    }
...
document.getElementById("btn-verify")?.addEventListener("click", async () => {
  ...
  const validation = validateVerifyInput(usuario, codigo);
  if (!validation.valid) {
    setMsg("verify-msg", validation.error);
    return;
  }
...
document.getElementById("btn-resend")?.addEventListener("click", async (e) => {
  ...
  if (!usuario) {
    setMsg("resend-msg", "Usuário não informado. Volte ao login e tente novamente.");
    return;
  }
  ...
  const result = await resendVerificationCode(usuario);

  link.innerText = originalText;
  link.style.pointerEvents = "";

  if (result.ok) {
    setMsg("resend-msg", "Código reenviado! Verifique sua caixa de entrada e spam.", true);
  } else {
    setMsg("resend-msg", result.error);
  }
});
